...
public class DatabaseService {
    ...
    public Connection getConnection(SessionInfo session) throws SQLException {
        ...
        try {
            ...
            boolean isPoolTimeout = (e instanceof SQLTransientConnectionException)
                    || (e.getMessage() != null && e.getMessage().contains("Connection is not available"));

            if (isPoolTimeout && ds instanceof HikariDataSource hikari) {
                try {
                    var mx = hikari.getHikariPoolMXBean();
                    if (mx != null) {
                        log.warn(
                                "Hikari pool exhausted: session_id={}, pool_name={}, active={}, idle={}, total={}, awaiting_threads={}, error={}",
                        ...
                        log.warn(
                                "Hikari pool exhausted: session_id={}, pool_name={}, error={}",
                                session != null ? session.getSessionId() : "",
                                hikari.getPoolName(),
                                e.getMessage()
                        );
                    ...
                    log.warn(
                            "Hikari pool exhausted: session_id={}, error={}",
                            session != null ? session.getSessionId() : "",
                            e.getMessage()
                    );
                }
    ...
    public void closeSession(String sessionId) {
        ...
        if (ds != null) {
            ...
            log.info("Closing session datasource: session_id={}, pool_name={}", sessionId, poolName);
            ds.close();
            long elapsedMs = (System.nanoTime() - startedAtNs) / 1_000_000L;
            log.info("Closed session datasource: session_id={}, pool_name={}, elapsed_ms={}", sessionId, poolName, elapsedMs);
            return;
        }

        long elapsedMs = (System.nanoTime() - startedAtNs) / 1_000_000L;
        log.info("No datasource found for session: session_id={}, elapsed_ms={}", sessionId, elapsedMs);
    }
